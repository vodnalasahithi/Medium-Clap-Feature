...
const ClapContainer = ({ children, setRef, handleClick, ...restProps }) => {
  return (
    <button ref={setRef} className="clap" onClick={handleClick} {...restProps}>
      {children}
    </button>
  );
};
  ...
  return (
    <span>
      <svg
        ...
        className={isClicked ? "icon" : "checked"}
      >
...
const ClapCount = ({ count, setRef, ...restProps }) => {
  return (
    <span ref={setRef} className="count" {...restProps}>
      + {count}
    </span>
  );
};
...
const CountTotal = ({ countTotal, setRef, ...restProps }) => {
  return (
    <span ref={setRef} className="total" {...restProps}>
      {countTotal}
    </span>
  );
};
    ...
    <div>
      ...
      >
        ...
        <ClapCount
          ...
          {...getCounterProps({ "aria-pressed": false })}
        />
        <CountTotal
      ...
      <section>
        ...
        <pre className="resetMsg">
          {uploadingReset ? `Uplaoding reset ${resetDep}...` : ""}
        </pre>
      </section>
    </div>
  );
